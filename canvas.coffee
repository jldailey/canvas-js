...
try
	random = (min,max) -> min + (Math.random() * max)
	...
	mixin = (fields) ->
		(args...) ->
			...
			for f,v of fields
				((f,v) -> # get a new scope for each iteration
	...
	velocity = mixin
		...
		draw: (ctx) ->
			...
			ctx.zip('drawLine').call @x,@y,@x+@vx,@y+@vy
	...
	circle = mixin
		init: (@r=30) ->
		draw: (ctx) -> ctx.zip('drawCircle').call(@x,@y,@r)
	...
	text = mixin
		init: (@text="Hello") ->
			...
			@h = 16
		draw: (ctx) ->
			ctx.zip('fillText').call(@text, @x, @y)
			@w = ctx.zip('measureText').call(@text).zip('width').first()
	...
	label = mixin
		init: (@label="hello") ->
		draw: (ctx) ->
			ctx.zip('fillText').call(labels[@label], @x, @y)
	...
	border = mixin
		draw: (ctx) ->
			ctx.zip('beginPath').call()
			ctx.zip('rect').call(@x,@y,@w,@h)
			ctx.zip('stroke').call()
			ctx.zip('closePath').call()
	...
	collide = mixin
		...
		draw: (ctx) ->
			for body in bodies
				...
				cpa = @clipPoint?(b)
				if cpa?
					ctx.zap('fillStyle', 'yellow').zip('drawCircle').call(cpa[0],cpa[1],2)
				else
					ctx.zap('fillStyle', 'purple').zip('drawCircle').call(@x+5,@y+5,5)
			if @showCollideNotice > 0
				ctx.zap('fillStyle', 'pink').zip('drawCircle').call(@x+5,@y-5,5)
	...
	class Scene
		constructor: (selector, items, @fps=60) ->
			...
			@ctx = $(selector).bind("mousemove", (evt) ->
				...
				labels["xy"] = "X: #{x} Y: #{y}"
			).zip("getContext").call("2d").map -> Object.Extend @,
				drawCircle: (x,y,r) ->
					@beginPath()
			...
			@last = +new Date
		tick: (dt) ->
			@ctx.zip('clearRect').call 0,0, @scene.boundx ? 320, @scene.boundy ? 240
	...
	$(document).ready ->
		...
		scene.resume()
		$("canvas").click -> scene.scene.forEach (x) -> force( randomInt(-10,10), -30, x )
